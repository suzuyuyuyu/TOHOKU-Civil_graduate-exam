...
\begin{ans*}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \begin{align}
      \int_{0}^{\pi/2} \frac{dx}{(1+\cos x)^2}
      &= \int_{0}^{1}\frac{1}{\biggl\{ 1+\biggl(\cfrac{1-t^2}{1+t^2}\biggr) \biggr\}^2}\frac{2}{(1+t^2)}\,dt \\
...
\begin{ans*}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \end{gather}
    であるので、Eq.~\eqref{eq:2020_autumn_diffintG}とEq.~\eqref{eq:2020_autumn_intG}の差をとって
    \begin{gather}
      G(t) - 2G'(t) - 2 = 0 \\
      2G'(t) - G(t) = 2 \\
    ...
    \dm{G(t)-2\neq 0}のとき、
    \begin{gather}
      \frac{G'(t)}{G(t)+2} = \frac{1}{2} \\
      \log |G(t)+2| = \frac{1}{2}t + C
    \end{gather}
    であるので、任意定数$A$を用いて一般解が
    \begin{gather}
      G(t) + 2 = Ae^\frac{t}{2}
    \end{gather}
    と得られる。\dm{G(t)+2=0}は\dm{A=0}のときである。
    よって一般解は
    \begin{gather}
      G(t) = Ae^\frac{t}{2} - 2
    \end{gather}
...
\begin{ans*}
  ...
  \end{gather}
  を積分因子としてとる。
  Eq.~\eqref{eq:2020_autumn_ODE}に積分因子$\grl(y)$をかけて
  \begin{gather}
...
\begin{ans*}
  ...
  \begin{enumerate}[label=(\arabic*)]
    ...
    \begin{enumerate}[label=(\roman*)]
      \item $\grl=\grl_1$のとき固有ベクトル$\bu_1$は
      \begin{gather}
        \bmat{
