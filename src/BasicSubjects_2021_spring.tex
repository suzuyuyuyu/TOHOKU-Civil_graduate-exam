...
\begin{ans*}
  ...
  \begin{enumerate}[label=(\arabic*)]
    ...
    ${}$
    \begin{figure}[H]\centering
      \includegraphics[width=.6\linewidth]{./src/fig/Basic/B_2021_spring_D.png}
    \end{figure}

    \item
    % % 別解です。
    % 積分領域$D$の$z$軸に垂直な面での切断面は以下のようになる。
    % \begin{figure}[H]\centering
    %   \includegraphics[width=.5\linewidth]{./src/fig/Basic/B_2021_spring_Dprime.png}
    % \end{figure}
    % この面を$z\in[0,1]$で積分すれば良い。
    与えられた領域$D$は$xy$平面を底面として
    球面までを高さとする立体であるので、
...
(3)は$\gra,\,\beta,\,\grg$ではなく$\xi,\,\eta,\,\grk$の
条件を問われているものと解釈しました。
