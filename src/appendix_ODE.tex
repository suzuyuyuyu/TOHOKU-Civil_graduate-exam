...
\begin{enumerate}[label=\Alph*.]
  ...
  \begin{enumerate}[label=\arabic*.]
    ...
    \begin{enumerate}[label=\bullet]
      ...
      \item \textbf{Bernoulli の微分方程式} \dm{p(x)y' + q(x)y = r(x)y^n}
      \item \textbf{完全微分方程式} \dm{P(x,y)dx + Q(x,y)dy = 0}
      \item \textbf{積分因子法} \dm{\grl(x,y)}
      \item \textbf{Lagrange/d'Alembert の微分方程式} \dm{y = xf(y') + g(y')}
    \end{enumerate}
    \item 二階以上
    \begin{enumerate}[label=\bullet]
      \item 階数低下法（階数低減法）
  ...
  \begin{enumerate}[label=\arabic*.]
    ...
    \item 二階以上（非定数係数）
    \begin{enumerate}[label=\bullet]
      \item \textbf{Euler の微分方程式} \dm{x^ny^{(n)} + a_{1}x^{n-1}y^{(n-1)} + \cdots + a_{n}y = f(x)}
      % \item Sturm--Liouville の微分方程式
    \end{enumerate}
    \item 二階以上（定数係数）
    \begin{enumerate}[label=\roman*.]
      \item 斉次
