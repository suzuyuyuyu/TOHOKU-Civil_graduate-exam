...
\begin{ans*}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    $f(x)$の$n$次導関数が
    \begin{gather}
      f^{(n)}(x) = \bigl(\sqrt{2}\bigr)^n e^x\sin\biggl(x + \frac{n\pi}{4}\biggr) \label{eq:2020_spring_goalfnx}
    \end{gather}
    となることを数学的帰納法を用いて示す。
    \begin{enumerate}[label=(\roman*)]
      ...
      \begin{align}
        ...
        &= \bigl(\sqrt{2}\bigr)^{1}e^x \sin\biggl(x + \frac{1\cdot\pi}{4}\biggr)
      \end{align}
      より$n=1$でEq.~\eqref{eq:2020_spring_goalfnx}は成り立つ。
      \item $n=k \,(\in \bbN)$でEq.~\eqref{eq:2020_spring_goalfnx}が成立すると仮定すると
      \begin{align}
        f^{(k)}(x) = \bigl(\sqrt{2}\bigr)^k e^x\sin\biggl(x + \frac{k\pi}{4}\biggr)
      \end{align}
      であり、これを微分した$k+1$階の導関数は
      \begin{align}
        f^{(k+1)}(x)
        &= \bigl(\sqrt{2}\bigr)^k e^x\biggl(\sin\biggl(x + \frac{k\pi}{4}\biggr) + \cos\biggl(x + \frac{k\pi}{4}\biggr)\biggr) \\
        &= \bigl(\sqrt{2}\bigr)^{k+1} e^x\sin\biggl(x + \frac{k\pi}{4}+\frac{\pi}{4}\biggr) \\
        &= \bigl(\sqrt{2}\bigr)^{k+1} e^x\sin\biggl(x + \frac{(k+1)\pi}{4}\biggr)
      \end{align}
      より$n=k+1$でも成り立つ。
    \end{enumerate}
    (i),(ii)と数学的帰納法より任意の自然数$n$に対して$n$次導関数が得られて、
    \begin{gather}
      f^{(n)}(x) = \bigl(\sqrt{2}\bigr)^n e^x\sin\biggl(x + \frac{n\pi}{4}\biggr)
    \end{gather}
    である。
  \end{enumerate}
...
\begin{ans*}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \begin{figure}[H]\centering
      \includegraphics[width=.6\linewidth]{./src/fig/Basic/B_2020_spring_ellipse.png}
    \end{figure}
...
\begin{ans*}
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \begin{gather}
      ...
      \log\left|\sin\frac{y}{x}\right| = -\log |x| + C \quad(\text{$C$は任意定数})
    \end{gather}
    \item
    \begin{align}
      \int y\cos x \,dx
